(() => {
  ...
  const rampStart = new Date(start.getFullYear(), 9, 1);
  const regularPlanStart = new Date(start.getFullYear(), 10, 1);
  const inPlanToday = dates.some(d => iso(d) === iso(now));
  const effectiveToday = inPlanToday ? now : dates[0];
  const save = () => localStorage.setItem(KEY, JSON.stringify(state));
  const weightForDate = date => {
    if (date < rampStart) return 0;
    if (date >= regularPlanStart) return 1;
    return (date - rampStart) / (regularPlanStart - rampStart);
  };
  const planModel = () => {
    const active = dates.filter(d => d >= effectiveToday);
    const actual = Object.values(state.facts).reduce((sum,v) => sum + (Number(v)||0), 0);
    const remaining = TARGET_TOTAL - actual;
    const totalWeight = active.reduce((sum,date) => sum + weightForDate(date), 0);
    const unit = totalWeight ? remaining / totalWeight : 0;
    return { remaining, unit, forDate: date => weightForDate(date) * unit };
  };
  const currentDay = () => dates.find(d => iso(d) === iso(effectiveToday));
  const update = () => {
    const plan = planModel(), rate = plan.forDate(effectiveToday), actual = Object.values(state.facts).reduce((a,v)=>a+(Number(v)||0),0), remaining = plan.remaining;
  ...
  const renderCalendar = plan => {
    ...
    for (const list of months.values()) {
      ...
      list.forEach(d => { const key=iso(d), past=d<effectiveToday, active=key===iso(effectiveToday), cell=document.createElement("div"); cell.className=`day ${past?"past":active?"today":"future"}`;
        const dailyPlan = plan.forDate(d);
        let body; if(past || active) body=`<input data-date="${key}" inputmode="decimal" aria-label="Факт за ${key}" value="${state.facts[key] ?? ""}" placeholder="факт">`; else body=`<span class="value">план ${money(dailyPlan)}</span>`;
        cell.innerHTML=`<span class="day-num">${d.getDate()}</span>${active?`<span class="value">план ${money(dailyPlan)}</span>`:""}${body}`; grid.append(cell);
  ...
  update();
  setInterval(() => updateClock(planModel().forDate(effectiveToday)), 1000);
  }
})();
